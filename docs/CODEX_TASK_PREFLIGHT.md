...
The default ready state is local `main`, clean worktree, and a synced-looking `main...origin/main` status. During PR validation on a feature branch, the helper is expected to halt with `stop_condition: branch_not_main`; that halt is the main-readiness gate working as designed.

Any `codex_task_preflight_status: halt` blocks branch, edit, commit, push, and PR progression until the reported `stop_condition` is resolved.
