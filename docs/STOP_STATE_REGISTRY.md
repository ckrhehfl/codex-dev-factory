...
| `STOPPED_ALLOWED_FILES_UNCLEAR` | Codex review loop | PR changed-file enumeration is incomplete or inconsistent with the expected changed-file count. | Sometimes | Re-read PR file metadata before allowing edits, commits, or pushes. | Codex review-loop runner, task contracts | Prevents ambiguous allowed-file scope. |
...
| `STOPPED_CODEX_REVIEW_NOT_ACKNOWLEDGED` | Codex review loop | A requested Codex review is not acknowledged in time. | Sometimes | Report the timeout and retry only when task contract allows another review request. | Codex review-loop runner, task contracts | Runner wait guard. |
| `STOPPED_CODEX_REVIEW_TIMEOUT` | Codex review loop | Codex does not produce a fresh latest-head result before the wait limit. | Sometimes | Report the latest observed review state and stop before merge. | Codex review-loop runner, task contracts | Prevents infinite review polling. |
| `STOPPED_COMMIT_FAILED` | Codex review loop | Creating the authorized review-fix commit fails. | Sometimes | Report git status and leave the worktree for owner inspection. | Codex review-loop runner, task contracts | No force push or cleanup is implied. |
...
| `STOPPED_GH_NOT_READY` | Codex review loop | `gh` is missing, unauthenticated, cannot read the repository, or cannot perform an authorized review-loop GitHub write. | Sometimes | Fix `gh` readiness or retry the authorized operation after confirming credentials without inspecting secrets. | Codex review-loop runner, task contracts | Does not authorize credential or auth-file inspection. |
...
| `STOPPED_LOOP_LIMIT_REACHED` | Codex review loop | The configured review-fix loop limit is reached. | Sometimes | Report remaining unresolved classifications and require a new task contract before continuing. | Codex review-loop runner, task contracts | Infinite-loop guard. |
...
| `STOPPED_NO_CURRENT_PR_FOUND` | Codex review loop | The current branch has no open GitHub pull request. | Sometimes | Checkout a PR branch or create a PR under an authorized task contract. | Codex review-loop runner, task contracts | Current-branch PR detection guard. |
| `STOPPED_ON_PROTECTED_BRANCH` | Codex review loop | The runner is invoked on a protected branch such as `main`, `master`, `develop`, or `release/*`. | Yes | Checkout the PR branch before running review-loop automation. | Codex review-loop runner, task contracts | Prevents self-operation on protected branches. |
| `STOPPED_OUT_OF_SCOPE_REVIEW_REQUEST` | Codex review loop | Unresolved review feedback requests work outside the current PR changed-file scope. | Yes | Report the out-of-scope feedback and wait for owner scope expansion. | Codex review-loop runner, task contracts | Out-of-scope threads may remain unresolved by policy. |
...
| `STOPPED_PR_METADATA_UNAVAILABLE` | Codex review loop | Required current PR metadata is unavailable, stale, incomplete, or not aligned with local HEAD. | Sometimes | Refresh PR state and retry from a clean branch matching the current PR head. | Codex review-loop runner, task contracts | Includes local-head versus PR-head mismatch. |
| `STOPPED_PUSH_FAILED` | Codex review loop | Pushing the current PR branch fails after an authorized review-fix commit. | Sometimes | Report the push failure and do not force push. | Codex review-loop runner, task contracts | Force push remains forbidden. |
...
| `STOPPED_REVIEW_COMMENTS_UNAVAILABLE` | Codex review loop | Review thread/comment data cannot be read or re-read before completion. | Sometimes | Retry after GitHub API availability is restored, without resolving or merging. | Codex review-loop runner, task contracts | Thread-state read guard. |
| `STOPPED_REVIEW_THREAD_RESOLVE_FAILED` | Codex review loop | An authorized review-thread resolution attempt fails. | Sometimes | Leave the thread unresolved and report the failed thread operation. | Codex review-loop runner, task contracts | Does not authorize resolving unfixed threads. |
...
| `STOPPED_UNSAFE_OR_FORBIDDEN_REVIEW_REQUEST` | Codex review loop | Unresolved review feedback asks for a forbidden or unsafe action or surface. | Yes | Report the unsafe request and do not attempt automated edits. | Codex review-loop runner, task contracts | Includes secret, settings, workflow, merge, force, and OMX boundaries. |
...
| `STOPPED_WORKTREE_DIRTY` | Codex review loop | The local worktree has uncommitted changes before review-loop automation starts. | Sometimes | Commit, stash, or clean unrelated work outside the runner before retrying. | Codex review-loop runner, task contracts | Runner-local alias for clean-worktree gate. |
